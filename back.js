...
export async function backGenerator(inputBuffer, description, IMAGINS) {
  ...
  const htmlImagins = wrappedImagins
    .map((line, index) => {
      return `<text x="55" y="${185 + (wrappedTitle.length * 50) + 40 + (index * 45)}" font-family="Poppins" font-size="28" font-weight="500" fill="black" text-anchor="start" dominant-baseline="middle">${escapeXML(line)}</text>`;
    })
    .join('\n');
